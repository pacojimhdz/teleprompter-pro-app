...
// 4. Funciones de Grabación de Video
function startRecording() {
    if (!stream) return;
    
    recordedChunks = [];
    
    // Si el filtro de audio está activo, capturamos el stream combinado
    let recordingStream = stream;
    if (audioContext && filterNode) {
        const destination = audioContext.createMediaStreamDestination();
        filterNode.connect(destination);
        
        // Combinar el video original con el audio filtrado
    ...
    mediaRecorder.onstop = () => {
        ...
        // Descarga automática temporal del archivo guardado
    ...
    btnStart.innerHTML = '<span class="btn-dot"></span> GRABANDO';
...
function stopRecording() {
    if (mediaRecorder && isRecording) {
        mediaRecorder.stop();
        isRecording = false;
        btnStart.innerHTML = '<span class="btn-dot"></span> GRABAR';
